    ...
    <section id="plans" className="bg-brand/40">
      <div className="mx-auto max-w-7xl px-6 py-20">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="mt-3 text-slate-300">Comparativo claro y elegante para que elijas el plan que mejor se adapta a tu empresa.</p>
        </div>

        {/* Cards (mobile-first) */}
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:hidden">
          {planMeta.map(p => (
            <article key={p.key} className="rounded-2xl border border-white/10 bg-white/5 p-6 flex flex-col">
              <header>
                <h3 className="text-xl font-semibold">{p.name}</h3>
                <p className="text-xs text-slate-400">{p.sub}</p>
                <p className="mt-3 text-3xl font-extrabold text-primary whitespace-nowrap">{p.price}</p>
              </header>
              <ul className="mt-4 space-y-2 text-sm text-slate-300">
                {rows.filter(r => !r.divider).slice(0, 7).map(r => (
                  <li key={r.label} className="flex items-start gap-2">
                    <span className="min-w-5 pt-1">
                    ...
                    <span className="flex-1">
                      ...
                      {typeof r.values[p.key] === 'string' ? <div className="text-slate-400">{r.values[p.key]}</div> : null}
                    </span>
                  </li>
                ))}
              </ul>
              <a href="#contact" className="mt-6 inline-flex justify-center items-center px-4 py-2 rounded-lg bg-primary hover:bg-primary-dark text-white font-medium">Me interesa</a>
            </article>
          ))}
        </div>
        ...
        <div className="mt-10 hidden lg:block overflow-x-auto">
          <div className="min-w-[960px] rounded-2xl border border-white/10 bg-white/5 overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-white/5">
                <tr>
                  <th className="text-left px-6 py-4 font-semibold">Descripción de los Servicios</th>
                  {planMeta.map(p => (
                    <th key={p.key} className="px-6 py-4 text-left">
              ...
              <tbody>
                {rows.map((r, idx) => r.divider ? (
                  ...
                  <tr key={r.label} className="border-t border-white/5">
                    ...
                    {planMeta.map(p => (
                      <td key={p.key} className="px-6 py-4"><CellValue value={r.values[p.key]} /></td>
                    ))}
                  </tr>
                ))}
                {/* Prices */}
                <tr className="border-t border-white/10 bg-white/5">
                  <td className="px-6 py-5 font-semibold">Inversión</td>
                  {planMeta.map(p => (
          ...
          <p className="mt-4 text-xs text-slate-400">Incluye: Semáforos (Dashboard/Tablero) y KPIs (indicadores de gestión). Análisis de Contabilidad según Régimen Tributario.</p>
        </div>
      </div>
    </section>
  )
